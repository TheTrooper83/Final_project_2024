# https://github.com/keras-team/keras-cv/blob/94b0a551d03ee9de3d81663e2c1a680cb113f7f1/keras_cv/src/layers/regularization/dropblock_2d.py#L23
...
class DropBlock2D(keras.layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        (
            ...
            value=block_size, n=2, name="block_size"
        )
        self.seed = seed
        # self._random_generator = tf.random.Generator.from_seed(self.seed)
        # self._random_generator = tf.keras.utils.set_random_seed(self.seed)
        self._random_generator = tf.random.set_seed(self.seed)
    ...
    def call(self, x, training=None):
        ...
        # _, height, width, _ = tf.unstack(tf.shape(x), 4)
        # _, height, width, _ = tf.experimental.numpy.split(tf.shape(x), 4)
        ...
        gamma = (
            self._rate
            * tf.cast(width * height, dtype="float32")
            / tf.cast(dropblock_height * dropblock_width, dtype="float32")
            / tf.cast(
        ...
        # w_i, h_i = tf.meshgrid(tf.experimental.numpy.arange(width), tf.experimental.numpy.arange(height))
        w_i, h_i = tf.meshgrid(tf.range(width), tf.range(height))
        valid_block = tf.experimental.numpy.logical_and(
        ...
        valid_block = tf.experimental.numpy.reshape(valid_block, [1, height, width, 1])

        random_noise = tf.random.uniform(
            tf.shape(x), seed=self._random_generator, dtype="float32"
        )
        valid_block = tf.cast(valid_block, dtype="float32")
        seed_keep_rate = tf.cast(1 - gamma, dtype="float32")
        block_pattern = (1 - valid_block + seed_keep_rate + random_noise) >= 1
        block_pattern = tf.cast(block_pattern, dtype="float32")
        ...
        percent_ones = tf.cast(tf.experimental.numpy.sum (block_pattern), "float32") / tf.cast(
            tf.experimental.numpy.size(block_pattern), "float32"
        )
        return (
            x
            / tf.cast(percent_ones, x.dtype)
            * tf.cast(block_pattern, x.dtype)
        )
